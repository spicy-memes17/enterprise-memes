...
class UploadForm(ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'video_url': Textarea(attrs={'class': 'form-control', 'rows': '1',
                                         'placeholder': 'URL that allows embedding (e.g. from YouTube)'}),
        }
...
class GroupForm(forms.Form):
    name = forms.CharField(
        widget=forms.Textarea(attrs={'class': 'form-control', 'rows': '1', 'placeholder': 'Enter Spicy Group Name'}))
...
class InviteForm(forms.Form):
    ...
    def clean(self):
        group = self.cleaned_data.get('group');
        exists = MemeGroup.objects.filter(group)
        if exists:
            raise forms.ValidationError('Group name already exists. Please choose another name.')
        return self.cleaned_data